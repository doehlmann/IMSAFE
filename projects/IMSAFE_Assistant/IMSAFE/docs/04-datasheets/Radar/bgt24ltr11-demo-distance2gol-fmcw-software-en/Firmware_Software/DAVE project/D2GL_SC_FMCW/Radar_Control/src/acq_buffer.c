									   ...
									   uint32_t num_sample_skip_count)
{
  ...
  obj.internals.adc_resolution = adc_resolution;
...
acq_buf_obj deep_copy_acq_buffer_obj(acq_buf_obj *p_src_obj, uint8_t *p_allocated_buffer, size_t allocated_buffer_size)
{
	acq_buf_obj obj;
	obj.status = not_initialized;

	p_src_obj->access_lock = 1;
	memcpy( &obj, p_src_obj, sizeof( acq_buf_obj));
	...
	if( p_allocated_buffer != NULL && allocated_buffer_size > 0)
	{
		  ...
		  if (p_allocated_buffer != NULL && allocated_buffer_size == 0)
		  {
		    ...
		    obj.internals.size_of_acq_buffer = 0;
		    obj.used_size_of_acq_buffer = 0;
		    p_src_obj->access_lock = 0;
		    return obj;
		  }

		  if (((obj.p_acq_buf != NULL) && (obj.internals.size_of_acq_buffer != 0)) && obj.internals.size_of_acq_buffer < obj.used_size_of_acq_buffer)
		  {
		    obj.status = allocation_buffer_too_small;
		    obj.p_acq_buf = NULL;
		    obj.internals.size_of_acq_buffer = 0;
		  ...
		  if(obj.p_acq_buf != NULL && obj.used_size_of_acq_buffer <= allocated_buffer_size)
		  {
			  p_src_obj->access_lock = 1;
		      memcpy(obj.p_acq_buf, p_src_obj->p_acq_buf, obj.used_size_of_acq_buffer);
		      p_src_obj->access_lock = 0;
		      obj.status = allocation_ok;
		  }
	}
