...
uint32_t ds_default_settings_check(void)
{
	...
	eep_status = ds_eeprom_read_settings(&eep_def_settings, sizeof(eep_def_settings), DS_EEPROM_ADRESS_DEFAULT_SETTING);
	...
	if (eep_status == EEPROM_STATUS_SUCCESS)
	{
			...
			def_algo_setting_crc 	!= eep_def_settings.def_dev_settings_crc)
		{
			...
			eep_status = ds_eeprom_write_settings(&eep_def_settings, sizeof(eep_def_settings), DS_EEPROM_ADRESS_DEFAULT_SETTING);
			ret_val = 1;
		}
	}
...
static char rf_board_id[] = RF_BOARD_ID;
...
uint32_t ds_init(void)
{
  ...
  shield_status = ds_eeprom_read_board_id(rf_board_id, sizeof(rf_board_id));
  if(shield_status != RF_SHIELD_BOARD_SUPPORTED)
  {
	...
	if( shield_status == RF_SHIELD_BOARD_NOT_PROGRAMMED)
	{
		...
		bsp_eeprom_time_delay(1000, BSP_TIMER_DELAY_MILLISECOND);
