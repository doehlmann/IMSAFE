/**
 * @file: EndpointRadarMcuStatus.c
 *
 * @brief: Radar endpoint containing functionality pertinent to Industrial
 *        radar demo kits, e.g. Distance2GoL, Position2Go etc.
 */
...
static void fill_status_resonse(device_status_response_t *p_status_response)
{
	...
	else
	{
		...
		p_status_response->error_msg_size = strlen(p_status_response->error_msg) + 1; // include the leading zereo!
	}
}
...
uint16_t send_device_status_response()
{
	if (device_status_endpoint != 0)
	{
		...
		fill_status_resonse(&status_response);
...
void ep_device_status_handle_change(uint8_t endpoint, void* context,
                                  	   uint32_t what)
{
